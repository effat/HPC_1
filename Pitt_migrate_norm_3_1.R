...
bbo2 <- function(fn, genomeLen,rMin,rMax, lower, upper, attType, sets,cLabel,DisplayFlag = TRUE, RandSeed, control = bbo.control(), ... ) {
  ...
  cat("entering in irace \n")
  ###optimize migration here
  
  my_instance<-population
  ...
  ##pop fitness by rank, best pop is costs[1], worst is cost[n]. But in migration, best pop has many species, or rank n
  pops<-c(1:P)
  pops_rank<-sapply(pops, function(x){(P-x)/P})
  ###sinusoid migration
  ##target runner to irace
  ...
  for( genIndex in 1:ctrl$maxGen ){
    ...
    #  lambda <-sapply(pops_rank, function(x){(1-x^tuned_a)^(1/tuned_a)})
    # mu     <-sapply(pops_rank, function(x){x^tuned_b})
    ...
    if(genIndex == 1){
      par( col="blue")
      plot(1:P, lambda, type="o", xlab="rank of pop", ylab = "lambda", main = "evolved norm ranking migration curve: lambda")
      plot(1:P, mu, type="o", xlab="rank of pop", ylab = "mu", main = "evolved norm ranking migration curve: mu")
